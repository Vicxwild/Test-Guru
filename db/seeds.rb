new_users = [
  User.create(name: 'Ivan', email: 'bla@foo.com'),
  User.create(name: 'Mira', email: 'mira@bla.com'),
  User.create(name: 'Marsi', email: 'marsi@bla.com')
]
...
questions_and_answers.keys.each_with_index do |name, index|
  ...
  test = new_users[index].tests.create(title: name, level: index, category_id: category.id)
